...
export interface AppNotification {
  ...
  createdAt: number;
  read: boolean;
  type: "homework" | "activity" | "progress" | "fees" | "circular" | "chat" | "system" | "bus";
  href?: string;
}
...
export function SchoolDataProvider({ children }: { children: ReactNode }) {
  ...
  }, [authReady, backend, user?.id, loadLocal, loadRemote, user]);

  // Soft realtime: poll while tab is visible (lightweight cadence).
  useEffect(() => {
    if (!backend || !user || !authReady) return;
    const tick = () => {
  ...
  }, [backend, user, authReady, loadRemote, pathname]);
