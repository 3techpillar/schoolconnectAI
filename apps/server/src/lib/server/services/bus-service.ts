import { BusRoute, busRouteToClient } from "@/lib/models/ops/BusRoute";
import { BusState } from "@/lib/models/ops/BusState";
import {
  DEFAULT_BUS_STOPS,
  DEFAULT_ROUTE_PATH,
...
export async function ensureBusRoute(
  ...
  let route = await BusRoute.findOne({ schoolId, routeId });
  if (!route) {
