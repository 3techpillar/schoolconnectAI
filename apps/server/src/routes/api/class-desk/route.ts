...
import {
  markCircularRead,
  publishCircular,
} from "@/lib/server/services/circular-service";
...
import {
  mergeAttendancePreservingLeave,
  studentKeysOnLeaveForDate,
...
export async function PATCH(req: Request) {
  ...
  if (body.setMark) {
    ...
    const onLeave = await studentKeysOnLeaveForDate({
      ...
      className,
      dateKey,
      roster: (desk.roster || []).map((s) => ({ key: s.key, name: s.name })),
    });
    const effective =
      onLeave.has(studentId) && mark !== "L" ? "L" : mark;
    const day = {
  ...
  if (body.attendance) {
    const existing =
      ((desk.attendanceByDay as Record<string, Record<string, string>>)[
        body.attendance.dateKey
      ] || {}) as Record<string, string>;
    const merged = await mergeAttendancePreservingLeave({
      schoolId: user.schoolId,
      className,
      dateKey: body.attendance.dateKey,
      roster: (desk.roster || []).map((s) => ({ key: s.key, name: s.name })),
      marks: body.attendance.marks,
      existing,
    });
    desk.attendanceByDay = {
      ...(desk.attendanceByDay as object),
      [body.attendance.dateKey]: merged,
    };
    desk.markModified("attendanceByDay");
  }
