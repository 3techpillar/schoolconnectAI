import { clientUserWithCapabilities, requireUser, withApiHandler } from "@/lib/server/http";
import { jsonOk } from "@/lib/server/auth";
import { User } from "@/lib/models/core/User";
import { listLinksForUser } from "@/lib/server/services/link-service";
...
async function patchHandler(req: Request) {
  ...
  if (user.role === "parent" && (body.parentAccess === "guardian" || body.parentAccess === "student")) {
    await User.collection.updateOne(
      { _id: user._id },
      { $set: { parentAccess: body.parentAccess } },
    );
  }

  const fresh = await User.findById(user._id);
  return jsonOk({ user: await clientUserWithCapabilities(fresh ?? user) });
}
