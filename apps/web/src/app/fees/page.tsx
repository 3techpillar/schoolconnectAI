...
import { EmptyState, ErrorBanner, LoadingBlock } from "@/components/shell/StatusUI";
  ...
  return (
    <PhoneShell subtitle={fees?.termLabel || "Fees"} title="Fees">
      ...
      {!loading && !fees && !error ? (
        <EmptyState
          title="No fee ledger"
          body="Fees will appear after your school links this account."
          tone="teal"
        />
      ) : null}
      ...
      {fees ? (
        <>
          <section className="list-hero list-hero-blue">
            <p className="list-hero-kicker">School fees</p>
            <h2 className="list-hero-title">{fees.termLabel}</h2>
            <p className="list-hero-body">
              Transparent dues, receipts, and payment history in one place.
            </p>
          </section>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      w.document.write(`<!doctype html><html><head><title>Receipt</title>
                        <style>
                          body{font-family:Inter,system-ui,sans-serif;padding:24px;color:#172554}
                          h1{font-size:18px;margin:0 0 8px}
                          p{margin:4px 0;font-size:14px;color:#64748b}
                          .amt{font-size:22px;font-weight:800;color:#172554;margin-top:12px}
                          .ok{color:#16a34a;font-weight:700}
                        </style></head><body>
                        <h1>SchoolConnect receipt</h1>
                        <p>${fees.termLabel}</p>
                        <p>${p.title}</p>
                        <p>${p.date}</p>
                        <p class="amt">${p.amount}</p>
                        <p class="ok">Paid</p>
                        <p style="margin-top:24px;font-size:12px">Demo receipt · print or save as PDF</p>
