...
import { isSchoolAdmin, ROLE_LABEL, useAuth } from "@/lib/providers/auth";
...
export function EnrollmentDesk({
  ...
  const { listUsers, updateUser, refreshDirectory } = useAuth();
  const {
    inviteTeacher,
    revokeInvite,
  ...
  const pendingJoinRequests = listUsers().filter((u) => {
    if (u.enrollmentStatus !== "pending") return false;
    if (
      !(
    ...
    if (actor.schoolId && u.schoolId) return u.schoolId === actor.schoolId;
    return u.school === actor.school;
  });
  ...
  return (
    <div className="space-y">
      {canInvite && (
        ...
        >
          ...
          <p className="text-11 muted" style={{ margin: 0 }}>
            Optional: teacher registers with this phone/email + invite code for
            instant access. Without a code they can still join as pending — approve
            them under Admin → Users.
          </p>
          <label>
      ...
      {canInvite && (
        <section className="card card-pad">
          <p className="font-semibold text-sm" style={{ margin: 0 }}>
            Join requests awaiting approval ({pendingJoinRequests.length})
          </p>
          <p className="text-11 muted mt-1" style={{ marginBottom: 0 }}>
            Users who selected your school without an invite code (or students
            still pending). Approve to unlock full app features.
          </p>
            ...
            {pendingJoinRequests.map((u) => (
              ...
              >
                ...
                <p className="text-11 muted" style={{ margin: "4px 0 0" }}>
                  {ROLE_LABEL[u.role]} · {u.identifier}
                  ...
                  <button
                    ...
                    onClick={() => {
                      void updateUser(u.id, {
                        ...
                        flash(`Approved ${u.name} — full access unlocked`);
                      });
                    }}
                  >
                    Approve access
