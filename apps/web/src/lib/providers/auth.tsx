...
export type { Role, UserProfile, ClassHistoryEntry };
...
interface AuthCtx {
  ...
  /** Replace directory from an already-fetched users list (avoids extra /api/users). */
  hydrateDirectory: (users: UserProfile[]) => void;
  updateUser: (
    id: string,
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const canListUsers = useCallback((role: Role | null | undefined) => {
    return (
      role === "admin" ||
      role === "super_admin" ||
      role === "principal" ||
  ...
  const persistSession = useCallback((u: UserProfile | null) => {
    userRef.current = u;
    setUser(u);
  ...
  const hydrateDirectory = useCallback((users: UserProfile[]) => {
    setDirectory(users);
  }, []);

  const refreshDirectory = useCallback(async () => {
    if (backendRef.current) {
      // Students/parents get 403 from /api/users — skip the round-trip.
      if (!canListUsers(userRef.current?.role)) {
        setDirectory([]);
        return;
      }
      try {
      ...
      try {
        ...
        if (ok) {
          try {
            ...
            if (cancelled) return;
            persistSession(res.user);
            // Do not block app ready on directory (staff-only; 403 for family roles).
            if (canListUsers(res.user.role)) {
          ...
          await refreshUser();
          if (!cancelled) setDirectory(Object.values(loadUsersMap()));
        }
      } finally {
          ...
          if (isSelf) {
            const res = await apiFetch<{ user: UserProfile }>("/api/me", {
              ...
              body: JSON.stringify({
                ...
                parentAccess: patch.parentAccess,
              }),
            });
            persistSession(res.user);
            setDirectory((prev) =>
...
export function canUseErp(user: UserProfile | null) {
