...
/** Offline demo: write L marks into teacher class localStorage. */
function applyApprovedLeaveOffline(
  leave: LeaveRequest,
  roster: Array<{ id: string; name: string }>,
) {
  if (typeof window === "undefined") return;
  const DESK_KEY = "sc_teacher_class_v1";
  try {
    const raw = localStorage.getItem(DESK_KEY);
    const desk = raw
      ? (JSON.parse(raw) as {
          roster?: Array<{ id: string; name: string }>;
          attendanceByDay?: Record<string, Record<string, string>>;
          circulars?: unknown[];
        })
      : { roster, attendanceByDay: {}, circulars: [] };
    const list = desk.roster?.length ? desk.roster : roster;
    const student = list.find(
      (s) => s.name.trim().toLowerCase() === leave.studentName.trim().toLowerCase(),
    );
    if (!student) return;
    const dates = daysBetween(leave.fromDate, leave.toDate);
    const byDay = { ...(desk.attendanceByDay || {}) };
    for (const dateKey of dates) {
      byDay[dateKey] = { ...(byDay[dateKey] || {}), [student.id]: "L" };
    }
    localStorage.setItem(
      DESK_KEY,
      JSON.stringify({ ...desk, roster: list, attendanceByDay: byDay }),
    );
  } catch {
    /* ignore */
  }
