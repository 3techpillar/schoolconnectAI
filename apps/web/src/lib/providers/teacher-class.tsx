...
export function TeacherClassProvider({ children }: { children: ReactNode }) {
  ...
  }, [backend, user?.className, user?.schoolId]);
  ...
  const markAllPresent = useCallback(
    (dateKey = day) => {
      const buildMarks = (roster: RosterStudent[], existing: Record<string, AttendMark>) => {
        const marks: Record<string, AttendMark> = {};
        roster.forEach((s) => {
          // Never overwrite an existing Leave mark locally; API also re-applies approved leave.
          marks[s.id] = existing[s.id] === "L" ? "L" : "P";
        });
        return marks;
      };
      const marks = buildMarks(state.roster, state.attendanceByDay[dateKey] || {});
      if (backend) {
        void apiFetch<{
          attendanceByDay: Record<string, Record<string, AttendMark>>;
        }>("/api/class-desk", {
      ...
      commit((prev) => ({
        ...
        attendanceByDay: {
          ...prev.attendanceByDay,
          [dateKey]: buildMarks(prev.roster, prev.attendanceByDay[dateKey] || {}),
        },
      }));
    },
    [commit, day, backend, user?.className, state.roster, state.attendanceByDay],
  );
