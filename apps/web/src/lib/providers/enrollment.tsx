...
/**
 * True when the signed-in user cannot use school-scoped features.
 * - API/Mongo mode: requires `schoolId`
 * - Local demo mode: school display name is enough
 * Super admin is platform-wide and is never gated here.
 */
...
export function hasSchoolAssigned(
  user: UserProfile | null,
  backend = true,
): boolean {
  return Boolean(user) && !needsSchoolAssignment(user, backend);
}
...
/** Routes allowed while school/enrollment setup is incomplete. */
export const LIMITED_ACCESS_PATHS = ["/pending", "/profile", "/auth"] as const;

export function isLimitedAccessPath(pathname: string): boolean {
  return LIMITED_ACCESS_PATHS.some(
