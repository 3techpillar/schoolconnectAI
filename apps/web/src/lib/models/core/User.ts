...
// Next.js hot reload can keep a compiled User model from before parentAccess existed.
if (!User.schema.path("parentAccess")) {
  User.schema.add({
    parentAccess: {
      type: String,
      enum: ["guardian", "student"],
...
export function userToClient(u: UserDoc) {
  return {
    ...
    parentAccess:
      u.role === "parent"
        ? ((u.parentAccess as "guardian" | "student" | undefined) === "student"
            ? "student"
            : "guardian")
        : undefined,
