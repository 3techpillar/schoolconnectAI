...
/** Cookie (`sc_session`) or `Authorization: Bearer` — shared by web + mobile. */
export async function getSessionFromCookiesOrBearer() {
  const jar = await cookies();
  let token = jar.get(AUTH_COOKIE)?.value;
  if (!token) {
    const h = await headers();
    const auth = h.get("authorization");
    if (auth) token = auth.replace(/^Bearer\s+/i, "") || undefined;
    if (!token) token = h.get("x-sc-session") || undefined;
  }
  if (!token) return null;
  try {
    return await verifySessionToken(token);
